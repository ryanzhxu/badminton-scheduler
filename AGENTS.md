...
3. Builds the layout/rounds with the existing `getLayout`/`generateRounds` (fixed at `AUTO_IMPORT_NUM_COURTS = 3`, `AUTO_IMPORT_ROUND_COUNT = 10`), generates a QR/share code, and marks it as the active/current schedule — same effect as a human clicking "Generate" + "Share".
